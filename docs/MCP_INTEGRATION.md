...
| Tool | Purpose |
|------|---------|
| `ingester_ingest` | **PRIMARY** - Ingest content with auto-classification |
...
#### `memory_store(content, content_type, source_path, metadata=None)` - DEPRECATED

> **DEPRECATED:** Use `ingester_ingest` instead. This tool is maintained for backward compatibility but returns a deprecation warning.
...
**Returns:** Deprecation warning + confirmation message with stored concept ID.

**Migration:** Use `ingester_ingest` instead:
```python
# DEPRECATED
memory_store(content="...", content_type="episteme", source_path="...")

# USE THIS INSTEAD
ingester_ingest(content="...", source_path="...", content_type_hint="episteme")
```
