...
last_updated: '2026-01-25T02:14:28'
---
# Investigation: INV Prefix Deprecation and Pruning
...
**Trigger:** Session 236 - Agent attempted to spawn INV-072 instead of WORK-012. Operator corrected: "we should not be spawning inv items anymore."

**Problem Statement:** Legacy ID prefixes (INV-*, E2-*, TD-*) are still being spawned despite TRD-WORK-ITEM-UNIVERSAL specifying WORK-XXX with type as field.

**Prior Observations:**
- TRD-WORK-ITEM-UNIVERSAL approved Session 218 - sequential IDs, type as field
- WORK-001 through WORK-011 created successfully
- Agent muscle memory still defaults to INV-* for investigations
- Survey-cycle and routing-gate still check `INV-*` prefix for routing
...
What code/skills/commands need updating to stop spawning legacy prefixes and route based on `type` field instead of ID prefix?
...
### In Scope
- Skills that route based on ID prefix (survey-cycle, routing-gate, implementation-cycle)
- Commands that spawn work items (/new-investigation, /new-work)
- Count of legacy items in queue

### Out of Scope
- Migrating existing legacy items (let them age out)
...
| Metric | Value | Source |
|--------|-------|--------|
| Files to examine | ~10 | Skills + commands |
| Hypotheses to test | 2 | Listed below |
| Expected evidence sources | Codebase | Grep for INV-* patterns |
| Estimated complexity | Low | Focused search |
...
|---|------------|------------|-------------|----------|
| **H1** | Routing logic (survey-cycle, routing-gate) uses `INV-*` prefix pattern | High | Grep for `INV-` in .claude/skills/ | 1st |
| **H2** | /new-investigation command scaffolds with INV-* prefix | High | Read .claude/commands/new-investigation.md | 2nd |
...
1. [ ] Grep for `INV-` pattern in .claude/skills/
2. [ ] Grep for `INV-` pattern in .claude/commands/
3. [ ] Count legacy items in queue: `just ready | grep -c "INV-\|E2-\|TD-"`

### Phase 2: Hypothesis Testing
4. [ ] Test H1: Read survey-cycle.md and routing-gate.md, find prefix-based routing
5. [ ] Test H2: Read new-investigation.md, find ID generation logic

### Phase 3: Synthesis
6. [ ] List files that need updating
7. [ ] Determine if migration needed or just code update
8. [ ] Spawn work items for fixes
...
|---------|------|-------|----------|-------|
| 236 | 2026-01-25 | HYPOTHESIZE | Started | Initial context and hypotheses |
