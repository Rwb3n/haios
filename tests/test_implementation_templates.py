...
# System Auto: last updated on: 2026-02-04T23:55:54
# Tests for implementation lifecycle phase templates (WORK-090)
"""
TDD tests for implementation lifecycle templates.
...
class TestImplementationTemplates:
    """Tests for .claude/templates/implementation/ phase templates."""

    impl_dir = Path(".claude/templates/implementation")
    phases = ["PLAN", "DO", "CHECK", "DONE"]

    def test_implementation_templates_directory_exists(self):
        """templates/implementation/ directory must exist."""
        assert self.impl_dir.is_dir(), "templates/implementation/ directory does not exist"

    def test_implementation_phase_templates_exist(self):
        """All four implementation phase templates must exist."""
        for phase in self.phases:
            template = self.impl_dir / f"{phase}.md"
            assert template.exists(), f"{phase}.md does not exist"

    def test_implementation_templates_have_contracts(self):
        """Each template must have input_contract and output_contract in frontmatter."""
        for phase in self.phases:
            template_path = self.impl_dir / f"{phase}.md"
    ...
    def test_implementation_templates_size_constraint(self):
