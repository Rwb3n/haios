    ...
    def check_budget(self):
        ...
        if current_cpu_time > self.budgets.max_cpu_seconds_per_plan:
            raise BudgetExceededError(
                f"CPU time limit exceeded: {current_cpu_time}s > {self.budgets.max_cpu_seconds_per_plan}s"
            )

        current_mem_bytes = self.process.memory_info().rss
        if current_mem_bytes > self.budgets.max_mem_bytes_per_plan:
            raise BudgetExceededError(
                f"Memory limit exceeded: {current_mem_bytes}B > {self.budgets.max_mem_bytes_per_plan}B"
            )
