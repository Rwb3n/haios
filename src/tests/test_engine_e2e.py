...
def e2e_project(tmp_path: Path) -> Path:
    ...
    (project_root / "project_templates/component.js.template").write_text("const component = {};")
...
def run_engine(project_root: Path, plan_id: str) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-m", "src.engine", "run-plan", plan_id, "--config", str(config_path)],
        capture_output=True,
        text=True,
...
def test_engine_e2e_happy_path(e2e_project: Path):
    ...
    (e2e_project / "os_root/initiatives/init_e2e/exec_plan_e2e_happy.txt").write_text(json.dumps(plan))
...
def test_engine_e2e_rejects_cycle(e2e_project: Path):
    ...
    (e2e_project / "os_root/initiatives/init_e2e/exec_plan_e2e_cycle.txt").write_text(json.dumps(plan))
...
def test_engine_e2e_rejects_path_traversal(e2e_project: Path):
    ...
    (e2e_project / "os_root/initiatives/init_e2e/exec_plan_e2e_traversal.txt").write_text(json.dumps(plan))
