...
from utils.validators import Validator, SchemaNotFoundError, SchemaValidationError

@pytest.fixture
def schema_dir(tmp_path: Path) -> Path:
    """Creates a temporary directory and a valid schema file within it."""
    schema_content = {
        "title": "Test Schema", "type": "object", "properties": {"name": {"type": "string"}}, "required": ["name"]
    }
