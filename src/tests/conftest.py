...
if str(_repo_root) not in _existing.split(os.pathsep):
    os.environ["PYTHONPATH"] = os.pathsep.join([str(_repo_root), _existing]) if _existing else str(_repo_root)
...
if str(_src_root) not in os.environ["PYTHONPATH"].split(os.pathsep):
    os.environ["PYTHONPATH"] = os.pathsep.join([str(_src_root), os.environ["PYTHONPATH"]])
...
try:
    ...
    def _safe_dump(self, obj):  # type: ignore
        try:
            ...
            return self.save_reduce(_rebuild_generic_obj, (obj.__class__, state), obj=obj)
