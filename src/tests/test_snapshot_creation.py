# ANNOTATION_BLOCK_START
{
  ...
  "payload": {
    "authors_and_contributors": [
        {"g_contribution": 215, "identifier": "Cody", "contribution_summary": "Creation (exec_plan_00005): Created integration test to provide evidence for the `CREATE_SNAPSHOT` task type, validating state file updates and artifact registration."}
    ],
    "internal_dependencies": ["src.core.config", "src.state_manager", "src.plan_runner"],
    "linked_issue_ids": []
  }
}
...
def snapshot_project(tmp_path):
    ...
    status_data = {"plan_id": "exec_plan_003", "payload": {"status": "DRAFT", "tasks_status": []}}
...
def test_snapshot_task_creates_and_registers_artifacts(snapshot_project):
    ...
    assert snapshot_path.exists(), "Snapshot file was not created in the correct directory."
    ...
    assert expected_snapshot_id in registry_tree, f"Snapshot artifact '{expected_snapshot_id}' was not registered."
    assert Path(registry_tree[expected_snapshot_id]["primary_filepath"]) == snapshot_path.relative_to(config.project_root)
    ...
    assert expected_snapshot_id in created_artifacts, "Snapshot ID was not logged in the plan's execution status."