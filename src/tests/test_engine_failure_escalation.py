# ANNOTATION_BLOCK_START
{
  ...
  "payload": {
    "authors_and_contributors": [
        ...
        {"g_contribution": 215, "identifier": "Cody", "contribution_summary": "Remediation (exec_plan_00005): Updated test to trigger readiness failure via `context_loading_instructions` path, exercising the newly hardened logic."}
    ],
...
def failure_path_project(tmp_path):
    ...
    status_data = {"plan_id": "exec_plan_002", "payload": {"status": "DRAFT", "status_details": {}, "tasks_status": []}}
...
def test_failure_escalates_blocker_and_registers_artifacts(failure_path_project):
    ...
    assert len(issue_files) == 1, "A single BLOCKER issue file should have been created."
    ...
    assert "Missing prerequisite file: non_existent_context_file.md" in issue_content["reason"]
    ...
    assert "human_attention_queue_artifact" in registry_tree, "Human attention queue artifact was not registered."