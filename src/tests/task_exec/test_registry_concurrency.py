# ANNOTATION_BLOCK_START
{
    ...
    "payload": {
        ...
        "authors_and_contributors": [
            {"g_contribution": 184, "identifier": "Successor Agent"}
        ],
        ...
        "scaffold_info": {"implementing_task_id": "T15_test_registry_concurrency"},
    },
}
...
def registry_writer_worker(
    config_dict: Dict[str, Any], mock_state_manager: Any, worker_id: int
):
    """
...
def test_concurrent_registry_writes_are_safe(tmp_path: Path):
    ...
    for i in range(num_processes):
        ...
        p = ctx.Process(
            target=registry_writer_worker,
            args=(config_dict_for_worker, mock_state_manager, i),
        )
        processes.append(p)
        p.start()
    ...
    for i in range(num_processes):
        ...
        assert (
            registered_artifacts[expected_artifact_id]["g_created_in_registry"] == 100
        )
