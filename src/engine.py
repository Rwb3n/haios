...
# ANNOTATION_BLOCK_START
{
  ...
  "payload": {
    ...
    "internal_dependencies": ["plan_runner_py_g101", "core_config_py_g146", "core_exceptions_py_g137", "util_config_loader_py_g61"],
...
from core.exceptions import ConfigError, PathEscapeError, WriteConflictError, PlannerError
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    run_plan.add_argument("plan_id", help="The plan identifier (filename without extension)")
    ...
    run_plan.add_argument("-v", "--verbose", action="store_true", help="Enable debug output")
    ...
    vault_init.add_argument("--key", required=True, help="The age identity key (bech32 format)")
    ...
    vault_add.add_argument("--scope", default="global", help="The scope of the secret (global, initiative, plan, agent)")
    vault_add.add_argument("--key", required=True, help="The age identity key (bech32 format)")
    ...
    vault_get.add_argument("--key", required=True, help="The age identity key (bech32 format)")

    vault_list = vault_sub.add_parser("list", help="List secrets in the vault")
    vault_list.add_argument("--key", required=True, help="The age identity key (bech32 format)")

    scaffold_parser = sub.add_parser("scaffold", help="Scaffold a new plan from a template")
    ...
    fsck_parser = sub.add_parser("registry-fsck", help="Check the integrity of the global registry map")
...
def _run_plan(plan_id: str, cfg: Config, bound_logger: Any, tracer: Any) -> None:  # pragma: no cover
...
def main(argv: Optional[list[str]] = None) -> None:  # pragma: no cover
    ...
    if args.cmd == "run-plan":
        ...
        print(f"Plan '{args.plan_id}' created from template '{args.template}' at {plan_path}")
        ...
        all_good = True
        for artifact_id, data in registry_data.get("payload", {}).get("artifact_registry_tree", {}).items():
            path_str = data.get("primary_filepath")
            if path_str:
                full_path = cfg.project_root / path_str
                if not full_path.exists():
                    print(f"  [FAIL] Artifact '{artifact_id}': file not found at '{full_path}'")
                    all_good = False
                else:
